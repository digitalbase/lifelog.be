...
import { Box, Button, Center, Flex, Portal, SimpleGrid, useBoolean, useFocusOnShow, VStack, useColorModeValue as mode } from '@chakra-ui/react';
import { HTMLMotionProps, motion, Variants } from 'framer-motion';
import FocusLock from 'react-focus-lock';
import { HiBookOpen, HiCloudDownload, HiCurrencyDollar, HiOutlineMenu, HiOutlineX, HiQuestionMarkCircle } from 'react-icons/hi';
...
const Backdrop = ({ show }: { show?: boolean }) => (
    <Portal>
        <motion.div
            ...
            variants={{ show: { opacity: 1, display: 'revert' }, hide: { opacity: 0, transitionEnd: { display: 'none' } } }}
            style={{ width: '100%', height: '100vh', position: 'absolute', background: 'rgba(0,0,0,0.2)', inset: 0 }}
        />
    </Portal>
)
...
export const MobileNav = () => {
    ...
    return (
        <>
            <Box as="button" type="button" p="1" fontSize="2xl" color="gray.600" onClick={toggle} display={{ base: 'block', lg: 'none' }}>
                <HiOutlineMenu />
            </Box>
            ...
            <Transition in={show}>
                ...
                <FocusLock disabled={!show} returnFocus>
                    <Box bg={mode('white', 'gray.700')} shadow="lg" rounded="lg" ref={ref} tabIndex={0} outline={0}>
                        <Box pt="5" pb="6" px="5">
